...
ID, DATUM, POPIS, PACIENT_ID, POJISTOVNA_ID


CREATE OR REPLACE TEMPORARY TABLE TEMP_id_navsteva AS

CREATE OR REPLACE TABLE id_navsteva AS
select PACIENT_ID, count(*), max(datum)
from SCH_CZECHITA.NAVSTEVY
group by PACIENT_ID
;


ID, JMENO, DATUM_NAROZENI, TELEFON, ADRESA

select JMENO,TELEFON
from pacienti
LEFT JOIN tte ON id = pacient_id
GROUP BY telefon
;
...
-- Stejně jako UNION (bez ALL) by fungnoval SELECT DISTINCT (také odstraňuje duplicitní řádky). Vyzkoušej si a zkontroluj počet záznamů výsledku:
...
-- Příklad jednoduchý na datech:
-- Spojíme tabulky NAVSTEVY a NAVSTEVY_2022
...
select weaptype1, WEAPTYPE1_TXT as weaptype_name
from teror
union
select weaptype2, WEAPTYPE2_TXT
from teror
union
select weaptype3, WEAPTYPE3_TXT
from teror
;
...
select attacktype1 as attacktype, attacktype1_txt as attacktype_name
from teror
where attacktype1 is not null

union
select attacktype2, attacktype2_txt
from teror
where attacktype2 is not null

union
select attacktype3, attacktype3_txt
from teror
...
navstevy - ID, DATUM, POPIS, PACIENT_ID, POJISTOVNA_ID
pacienti - ID, JMENO, DATUM_NAROZENI, TELEFON, ADRESA


select p.ID,p.TELEFON,n.POPIS, n.datum

from pacienti as p
inner join navstevy as n
...
SELECT IYEAR, COUNT (*) OVER (PARTITION BY IYEAR)
FROM TEROR
Where IYEAR in (2018,2019,2020)
--group by IYEAR
;
...
from teror
QUALIFY count(*) over (partition by IYEAR) >=14000
;
...
from
(
select SUM(NKILL) OVER (PARTITION BY IDATE) as POCET_OBETI_KUMULATIVNE, *
from teror
order by IYEAR, IDATE
)
WHERE POCET_OBETI_KUMULATIVNE <= 100
;
...
Підрахуйте, скільки жертв мала кожна терористична група (GNAME) (NKILL) - назвіть стовпець POCET_OBETI (використовуючи GROUP BY). Відфільтруйте групу "Невідомі". Відсортуйте за кількістю жертв (NULL-значення будуть в кінці).
--> 2. Do stejného příkazu spočítejte celkový počet (= za celý dataset) zabitých obětí (NKILL) pomocí WINDOW funkce. Pojmenujte jako POCET_OBETI_CELKEM. Přidejte také sloupec ukazující podíl zabitých (POCET_OBETI / POCET_OBETI_CELKEM) - pojmenujte jako PROCENTO_OBETI.
 У цій же команді обчисліть загальну кількість (= для всього набору даних) вбитих жертв (NKILL) за допомогою функції WINDOW. Назвіть її POCET_VICTES_CELKEM. Також додайте стовпчик, що показує частку вбитих (POCET_OBETI / POCET_OBETI_CELKEM) - назвіть його PROCENTO_OBETI.
--> 3. Ve stejném příkazu filtrujte pouze organizace, které se na celkovém počtu obětí podílelo alespoň 7 procenty (PROCENTO_OBETI >= 0.07).
 У тій самій команді відфільтруйте лише організації, на які припадає щонайменше 7 відсотків від загальної кількості жертв (PROCENTO_OBETI >= 0.07).


SELECT
  GNAME
, SUM(NKILL) AS POCET_OBETI
, SUM(POCET_OBETI) OVER () AS POCET_OBETI_CELKEM
